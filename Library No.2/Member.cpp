...
void Member::borrowBook(Book& book)
{
    ...
    vector<Book>& bookList = this->getBookList();
    for (int i = 0; i < bookList.size(); i++) {
        if (book == bookList[i]) bookPosition = i;
    }
    borrowList.push_back(book);
    bookList[bookPosition].setAvailabilityStatus(false);
...
void Member::returnBook(Book& book)
{
    ...
    if (bookInBorrowedListPosition + 1 == borrowList.size()) borrowList.pop_back();
    else {
...
void Member::subscribeCollection(int collectionPosition) {
    vector<Collection>& collectionList = this->getCollectionList();
    Collection& collection = collectionList[collectionPosition];
    subscribedCollectionList.push_back(collection);
